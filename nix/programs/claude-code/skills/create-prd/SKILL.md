...
ユーザーの依頼がステータス変更や内容追記であれば手順8へ進む。新規作成なら手順2へ。
既に同じ領域を扱うPRDがある場合は、新規作成せずそのPRDの更新を提案する。
...
- `Shipped` にする: `Status` 行と `ADRs` 行を更新する
- 本文を更新する: PRDは `Shipped` 後も現状に追従させる。挙動が変わったら `Goals` と
  `Requirements` を現在のリポジトリの記述に書き換える。計画時点の内容はgit履歴に残るので、
  古い記述をファイル内に残さない
- `Dropped` にする: `Status` 行を変更し、`Open questions` に中止理由を1行残す。本文はそのまま
