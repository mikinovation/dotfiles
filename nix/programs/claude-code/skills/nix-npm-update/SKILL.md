...
`PKG` と `NPM_PKG` を切り替えて vue-language-server / vue-typescript-plugin の両方に同手順を適用する。

#### 3.1. 手順1のJSONから値を取り出す

```bash
PKG=vue-language-server                  # または vue-typescript-plugin
NPM_PKG=@vue/language-server             # または @vue/typescript-plugin
JSON=/tmp/vls.json                       # または /tmp/vtp.json
